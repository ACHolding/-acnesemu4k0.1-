...
class Mapper19(_MapperBase):
    """Namco 163 — minimal PRG/CHR banking for boot."""
    __slots__ = ("prg_bank", "chr_bank")
...
class Mapper68(_MapperBase):
    """Sunsoft FME-7 — boot stub (UNROM-style PRG)."""
    __slots__ = ("bank",)
...
class CPU:
    ...
    def am_abs(self):
        self._addr = self.r16(self.pc); self.pc = (self.pc + 2) & 0xFFFF; self._pxc = False
    def am_abx(self):
...
def _build_table():
    ...
    for c, am, cy in (
        ...
        (0x37,"am_izy",5),(0x3B,"am_aby",4),(0x3F,"am_aby",4),
    ):
        u(c, am, "op_rla", cy)
    # SRE (LSR + EOR)
    for c, am, cy in (
        (0x43,"am_izx",8),(0x47,"am_izy",5),(0x4F,"am_abs",4),(0x53,"am_izy",5),
        (0x57,"am_izy",5),(0x5B,"am_aby",4),(0x5F,"am_aby",4),
    ):
        u(c, am, "op_sre", cy)
    # RRA (ROR + ADC)
    for c, am, cy in (
                ...
                for ry in range(sp_h):
                    ...
                    for fx in range(8):
                        ...
                        if px_curr < 0 or px_curr >= W:
                            continue
                        if behind and bg_pix[row_off + px_curr] != bd_idx:
                            continue
