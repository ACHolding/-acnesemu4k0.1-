...
class Mapper11(_MapperBase):
    """Color Dreams — CHR bank @ $8000 writes."""
    __slots__ = ("chr_bank",)
    ...
    def prg_write(self, addr: int, val: int) -> None:
        if addr >= 0x8000:
            n = max(1, len(self.rom.chr) // 0x2000) if self.rom.chr else 1
            self.chr_bank = val & (n - 1)
...
class Mapper66(_MapperBase):
    ...
    def prg_read(self, addr: int) -> int:
        prg = self.rom.prg
        return prg[(self.bank * 0x8000 + (addr - 0x8000)) % len(prg)]

    def prg_write(self, addr: int, val: int) -> None:
        if addr >= 0x8000:
            n = max(1, len(self.rom.prg) // 0x8000)
            self.bank = val & (n - 1)
...
_MAPPER_MMC2 = {9, 112}
_MAPPER_MMC3 = {
    4, 12, 44, 45, 47, 49, 52, 74, 88, 91, 114, 115, 118, 119, 165,
    191, 192, 193, 194, 195, 206, 209, 215, 249, 250, 254,
}
_MAPPER_MMC4 = {10}
_MAPPER_CNROM = {3, 185}
_MAPPER_UNROM = {
...
def build_mapper(bus: "Bus", rom: INES) -> _MapperBase:
    ...
    # Heuristic fallbacks for odd mapper IDs
...
class App:
    ...
    def _present_frame(self, rgb: bytes | None) -> None:
        self._refresh_state()
        if rgb: self._canvas_show_rgb_buffer(256, 240, rgb, "")
    ...
    def _refresh_state(self):
        ...
        c = self.nes.cpu
        self.lbl_cpu.configure(text=f"PC:{c.pc:04X}  A:{c.a:02X} X:{c.x:02X} Y:{c.y:02X}  SP:{c.sp:02X}  P:{c.p:02X}  cyc:{c.cycles}")
    ...
    def ppu_viewer(self):
        ...
        w.title("PPU Viewer")
        cv = tk.Canvas(w, width=512, height=256, bg=BG)
        cv.pack()
        # Preview decoded elements logic

    def hex_editor(self):
        w = tk.Toplevel(self.root, bg=BG)
        w.title("Hex Editor")
        txt = tk.Text(w, width=70, height=20, bg=BG, fg=FG)
        txt.pack()
        ram = self.nes.bus.ram
        txt.insert("1.0", "\n".join(f"{row:04X}: " + " ".join(f"{ram[row+i]:02X}" for i in range(16)) for row in range(0, 0x0800, 16)))

    def debugger(self):
        w = tk.Toplevel(self.root, bg=BG)
        w.title("Debugger")
        self._lbl(w, "Debugger active").pack()
