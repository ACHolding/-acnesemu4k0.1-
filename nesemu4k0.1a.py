...
def _apply_tetris_nrom_header_fix(
    rom: "INES",
    flags6: int,
    flags7: int,
    prg_banks: int,
    ...
    """Fix corrupt iNES headers on NROM Tetris dumps (garbled text if mapper/mirror wrong)."""
    if prg_banks < 1 or prg_banks > 2 or chr_banks > 2:
        return
        
    # Check for classic corrupt headers (bogus mapper-1 bits / false NES 2.0 signatures)
    is_corrupt_tetris = False
    if flags6 == 0x10 and flags7 in (0x00, 0x08):
        is_corrupt_tetris = True
    elif flags6 == 0x10 and ines1_mapper == 1 and chr_banks >= 1:
        is_corrupt_tetris = True

    if is_corrupt_tetris:
        rom.mapper = 0
        rom.mirror = "V"
        # CRITICAL FIX: Explicitly fix the bank counts so down-stream data array loading 
        # doesn't slice the source file buffers with invalid offset lengths.
        rom.prg_banks = prg_banks if prg_banks in (1, 2) else 2
        rom.chr_banks = 1 if chr_banks >= 1 else 0
...
class INES:
    ...
    def __init__(self, path: str):
        ...
        ines1_mapper = ((flags6 >> 4) & 0x0F) | (flags7 & 0xF0)
        self.mapper = ines1_mapper
        nes2 = len(data) >= 32 and (flags7 & 0x0C) == 0x08 and (flags7 & 0xF0) == 0
        if nes2:
            ext_mapper = (data[8] | (data[9] << 8)) & 0xFFF
            # Reject false NES 2.0: byte7=0x08 with huge mapper but classic PRG/CHR sizes
            if ext_mapper >= 48 and prg_banks <= 4 and chr_banks <= 4:
                self.mapper = ines1_mapper
            else:
                self.mapper = ext_mapper
            if data[4] == 0:
                prg_banks = max(1, (data[10] | (data[11] << 8) | (data[12] << 16)) // 16384)
            if data[5] == 0:
                chr_banks = max(0, (data[13] | (data[14] << 8) | (data[15] << 16)) // 8192)
                
        # Run header fixes before calculation of file slice offsets
        _apply_tetris_nrom_header_fix(self, flags6, flags7, prg_banks, chr_banks, ines1_mapper)
...
class _MapperBase:
    ...
    def load_battery(self, sav_path: str) -> None:
        if not os.path.isfile(sav_path):
            return
        try:
            with open(sav_path, "rb") as f:
                data = f.read(8192)
            self.wram[:len(data)] = data
        except OSError:
            pass

    def save_battery(self, sav_path: str) -> None:
        try:
...
def _battery_sav_path(rom_path: str) -> str:
    return os.path.splitext(rom_path)[0] + ".sav"
...
class Mapper4(_MapperBase):
    ...
    def step_scanline(self, sl: int) -> None:
        if sl < 0 or sl >= 240 or not self.irq_enabled:
            return
...
class Mapper5(_MapperBase):
    """MMC5 partial — PRG slots, CHR banking, mirror layout."""
    __slots__ = ("prg", "chr", "mirror_mode")
    ...
    def prg_write(self, addr: int, val: int) -> None:
        val &= 0xFF
        if 0x5105 <= addr <= 0x5107:
            self.mirror_mode = val & 3
            if val & 3 == 0:
                self.bus.ppu_mirror = self.rom.mirror
            elif val & 3 == 1:
            ...
            nk = max(1, len(self.rom.chr) // 0x400) if self.rom.chr else 1
            self.chr[addr - 0x5120] = val & (nk - 1)
    ...
    def chr_read(self, addr: int) -> int:
        ...
        cl = len(c)
        slot = (addr >> 10) & 0x0F
        if slot < len(self.chr):
            bank = self.chr[slot]
        else:
            bank = 0
        return c[(bank * 0x400 + (addr & 0x3FF)) % cl]
...
class Mapper30(_MapperBase):
    """UNROM-512 (Mapper 30) — large UxROM configuration."""
    __slots__ = ("bank",)
...
class Mapper9(_MapperBase):
    """MMC2 / PxROM — latch CHR (Punch-Out!! style latching)."""
    ...
    def _mmc2_bank(self, addr: int) -> int:
        a = addr & 0x1FFF
        low = a & 0x0FF0
        if low == 0x0FD0:
            return self.latch_fd
        if low == 0x0FE0:
            return self.latch_fe
        return self.latch_fd if a < 0x1000 else self.latch_fe
    ...
    def chr_read(self, addr: int) -> int:
        ...
        if not c:
            return 0
        cl = len(c)
        bank = self._mmc2_bank(addr)
        return c[(bank * 0x1000 + (addr & 0x0FFF)) % cl]


class Mapper10(_MapperBase):
    """MMC4 / FxROM — latch CHR logic."""
    ...
    def _mmc4_bank(self, addr: int) -> int:
        a = addr & 0x1FFF
        if self.chr_mode == 0:
            low = a & 0x0FF8
            if low == 0x0FD8:
            ...
            if low == 0x0FE8:
                return self.latch_1
            return self.latch_0 if a < 0x1000 else self.latch_1
        return self.latch_1 if (a & 0x1000) else self.latch_0
    ...
    def chr_read(self, addr: int) -> int:
        ...
        if not c:
            return 0
        cl = len(c)
        bank = self._mmc4_bank(addr)
        return c[(bank * 0x1000 + (addr & 0x0FFF)) % cl]


class _MapperBankedPRG(_MapperBase):
    """Generic 16K switchable @ $8000 (VRC2/4/7 boot stubs)."""
    __slots__ = ("bank",)
...
class Mapper19(_MapperBase):
    """Namco 163 — minimal PRG/CHR banking for boot sequence."""
    __slots__ = ("prg_bank", "chr_bank")
    ...
    def prg_write(self, addr: int, val: int) -> None:
        if 0x8000 <= addr < 0xC000:
            self.prg_bank = val & 0x3F
        elif 0xE000 <= addr < 0xF000:
            cn = max(1, len(self.rom.chr) // 0x0400) if self.rom.chr else 1
            self.chr_bank = val & (cn - 1)

    def chr_read(self, addr: int) -> int:
        c = self.rom.chr
        if not c:
            return 0
        return c[(self.chr_bank * 0x0400 + (addr & 0x03FF)) % len(c)]


class Mapper68(_MapperBase):
    """Sunsoft FME-7 — boot stub (UNROM-style PRG map)."""
    __slots__ = ("bank",)
...
# Mapper ID aliases mapping to implementations
_MAPPER_MMC1 = {1, 155}
_MAPPER_MMC2 = {9, 112}
_MAPPER_MMC3 = {
...
_MAPPER_UNROM = {
    ...
    70, 71, 72, 73, 75, 78, 79, 87, 89, 94, 95, 97, 99, 101,
    107, 108, 109, 113, 140, 152, 154, 156, 159, 180, 206, 207, 245,
}
_MAPPER_AXROM = {7, 34, 47, 58, 93, 146, 147, 149, 196}
_MAPPER_GXROM = {66}
_MAPPER_CDREAMS = {11}
_MAPPER_UNROM512 = {30}
_MAPPER_NAMCO163 = {19, 210}
_MAPPER_VRC24 = {21, 23, 25}
_MAPPER_VRC6 = {24, 26}
_MAPPER_FME7 = {68, 69}
_MAPPER_SUNSOFT = {184}
...
def build_mapper(bus: "Bus", rom: INES) -> _MapperBase:
    ...
    if m in _MAPPER_MMC3 or m in _MAPPER_SUNSOFT:
        return Mapper4(bus, rom)
    if m in _MAPPER_CNROM:
        return Mapper3(bus, rom)
    if m in _MAPPER_UNROM512:
        return Mapper30(bus, rom)
    if m in _MAPPER_NAMCO163:
        return Mapper19(bus, rom)
    if m in _MAPPER_VRC24 or m in _MAPPER_VRC6:
        return _MapperBankedPRG(bus, rom)
    if m in _MAPPER_FME7:
        return Mapper68(bus, rom)
...
class Bus:
    ...
    def read(self, addr: int) -> int:
        ...
        if addr < 0x8000:
            if self.mapper: return self.mapper.wram_read(addr)
            return 0
    ...
    def write(self, addr: int, val: int) -> None:
        ...
        if addr == 0x4016:
            ...
            return
        if 0x6000 <= addr < 0x8000:
            if self.mapper: self.mapper.wram_write(addr, val)
            return
...
class CPU:
    ...
    def r16(self, a):  return self.r(a) | (self.r((a + 1) & 0xFFFF) << 8)
    ...
    def am_imp(self): self._pxc = False
    def am_acc(self): self._pxc = False
    def am_imm(self): self._addr = self.pc; self.pc = (self.pc + 1) & 0xFFFF; self._pxc = False
    def am_zp (self): self._addr = self.r(self.pc); self.pc = (self.pc + 1) & 0xFFFF; self._pxc = False
    def am_zpx(self): self._addr = (self.r(self.pc) + self.x) & 0xFF; self.pc = (self.pc + 1) & 0xFFFF; self._pxc = False
    def am_zpy(self): self._addr = (self.r(self.pc) + self.y) & 0xFF; self.pc = (self.pc + 1) & 0xFFFF; self._pxc = False
    def am_abs(self): self._addr = self.r16(self.pc); self.pc = (self.pc + 2) & 0xFFFF; self._pxc = False
    def am_abx(self):
        base = self.r16(self.pc); self.pc = (self.pc + 2) & 0xFFFF
        addr = (base + self.x) & 0xFFFF
        self._pxc = (base & 0xFF00) != (addr & 0xFF00)
        self._addr = addr
    def am_aby(self):
        base = self.r16(self.pc); self.pc = (self.pc + 2) & 0xFFFF
    ...
    def _pxc_add(self):
        if self._pxc: self.cycles += 1

    def op_lda(self): self.a = self.r(self._addr); self.setNZ(self.a); self._pxc_add()
    def op_ldx(self): self.x = self.r(self._addr); self.setNZ(self.x); self._pxc_add()
    def op_ldy(self): self.y = self.r(self._addr); self.setNZ(self.y); self._pxc_add()
    ...
    def op_and(self): self.a &= self.r(self._addr); self.setNZ(self.a); self._pxc_add()
    def op_ora(self): self.a |= self.r(self._addr); self.setNZ(self.a); self._pxc_add()
    def op_eor(self): self.a ^= self.r(self._addr); self.setNZ(self.a); self._pxc_add()
    ...
    def _cmp(self, reg):
        ...
        self.setF(C, reg >= m)
        self.setNZ(d & 0xFF)
    def op_cmp(self): self._cmp(self.a); self._pxc_add()
    ...
    def op_asl_a(self): self.setF(C, self.a & 0x80); self.a = (self.a << 1) & 0xFF; self.setNZ(self.a)
    def op_asl(self):
        m = self.r(self._addr); self.setF(C, m & 0x80)
        m = (m << 1) & 0xFF;    self.w(self._addr, m); self.setNZ(m)
    def op_lsr_a(self): self.setF(C, self.a & 1);  self.a >>= 1;       self.setNZ(self.a)
    def op_lsr(self):
    ...
    # --- Unofficial stable instruction mappings ---
    def op_lax(self):
    ...
    def op_sax(self): self.w(self._addr, self.a & self.x)
    def op_slo(self):
        m = self.r(self._addr)
        self.setF(C, m & 0x80)
        m = (m << 1) & 0xFF
...
def _build_table():
    ...
    def u(code, am, op, cyc):
        if t[code] is None: s(code, am, op, cyc)

    # SLO (ASL + ORA)
    for c, am, cy in ((0x03,"am_izx",8),(0x07,"am_izy",5),(0x0F,"am_abs",4),(0x13,"am_izy",5),(0x17,"am_izy",5),(0x1B,"am_aby",4),(0x1F,"am_aby",4)):
        u(c, am, "op_slo", cy)
    # RLA (ROL + AND)
    for c, am, cy in ((0x23,"am_izx",8),(0x27,"am_izy",5),(0x2F,"am_abs",4),(0x33,"am_izy",5),(0x37,"am_izy",5),(0x3B,"am_aby",4),(0x3F,"am_aby",4)):
        u(c, am, "op_rla", cy)
    # SRE (LSR + EOR)
    for c, am, cy in ((0x43,"am_izx",8),(0x47,"am_izy",5),(0x4F,"am_abs",4),(0x53,"am_izy",5),(0x57,"am_izy",5),(0x5B,"am_aby",4),(0x5F,"am_aby",4)):
        u(c, am, "op_sre", cy)
    # RRA (ROR + ADC)
    for c, am, cy in ((0x63,"am_izx",8),(0x67,"am_izy",5),(0x6F,"am_abs",4),(0x73,"am_izy",5),(0x77,"am_aby",4),(0x7B,"am_aby",4),(0x7F,"am_aby",4)):
        u(c, am, "op_rra", cy)
    # SAX
    for c, am, cy in ((0x83,"am_izx",6),(0x87,"am_izy",5),(0x8F,"am_abs",4),(0x97,"am_izy",5)):
        u(c, am, "op_sax", cy)
    # LAX
    for c, am, cy in ((0xA3,"am_izx",6),(0xA7,"am_imm",2),(0xAF,"am_abs",4),(0xB3,"am_izy",5),(0xB7,"am_zpy",4),(0xBF,"am_aby",4)):
        u(c, am, "op_lax", cy)
    # DCP
    for c, am, cy in ((0xC3,"am_izx",8),(0xC7,"am_izy",5),(0xCF,"am_abs",4),(0xD3,"am_izy",5),(0xD7,"am_zpx",6),(0xDF,"am_aby",4)):
        u(c, am, "op_dcp", cy)
    # ISB
    for c, am, cy in ((0xE3,"am_izx",8),(0xE7,"am_izy",5),(0xEF,"am_abs",4),(0xF3,"am_izy",5),(0xF7,"am_izy",5),(0xFB,"am_aby",4)):
        u(c, am, "op_isb", cy)
    ...
    u(0xEB, "am_imm", "op_sbc", 2)
    u(0xCB, "am_imm", "op_sbx", 2)

    for code in (0x02, 0x1A, 0x3A, 0x5A, 0x7A, 0x8B, 0x9B, 0xDA, 0xBB, 0xFA):
        u(code, "am_imp", "op_nop", 2)
    for code in (0x04, 0x44, 0x64, 0x80, 0x82, 0x89, 0xC2, 0xE2, 0x14, 0x34, 0x54, 0x74, 0xD4, 0xF4):
        u(code, "am_imm", "op_nop", 2)
    for code in (0x0C, 0x1C, 0x3C, 0x5C, 0x7C, 0xDC, 0xFC, 0x92, 0x93, 0x9C, 0x9E, 0x9F, 0xB2, 0xDB, 0xFF):
        u(code, "am_abs", "op_nop", 3)
    return t

def _opcode_audit() -> list[int]:
    official = (
        0x00,0x01,0x05,0x06,0x08,0x09,0x0A,0x0D,0x0E,0x10,0x11,0x15,0x16,0x18,0x19,0x1D,0x1E,0x20,
        0x21,0x24,0x25,0x26,0x28,0x29,0x2A,0x2C,0x2D,0x2E,0x30,0x31,0x35,0x36,0x38,0x39,0x3D,0x3E,
        ...
        0xBA,0xBC,0xBD,0xBE,0xC0,0xC1,0xC4,0xC5,0xC6,0xC8,0xC9,0xCA,0xCC,0xCD,0xCE,0xD0,0xD1,0xD5,
        0xD6,0xD8,0xD9,0xDD,0xDE,0xE0,0xE1,0xE4,0xE5,0xE6,0xE8,0xE9,0xEA,0xEC,0xED,0xEE,0xF0,0xF1,
        0xF5,0xF6,0xF8,0xF9,0xFD,0xFE,
    )
    t = CPU._table
    return [c for c in official if t[c] is None]

CPU._table = _build_table()
assert not _opcode_audit(), f"missing official opcodes: {_opcode_audit()}"


# =============================================================================
#   PPU Core
# =============================================================================
# Standard 2C02 NTSC RGB Palette layout
NES_PALETTE = [
...
class PPU:
    __slots__ = (
        ...
        "_fine_x", "_scroll_t", "_scroll_fine_x", "oam_addr", "read_buffer",
        "_vblank", "_sprite0_hit", "_nmi_sent",
        "_snap_v", "_snap_fine_x", "_snap_ctrl", "_snap_mask",
        "_snap_nt", "_snap_pal", "_snap_oam", "_snap_valid",
        "nes", "bus",
    )
    ...
    def __init__(self):
        ...
        self._snap_nt = bytearray(2048)
        self._snap_pal = bytearray(32)
        self._snap_oam = bytearray(256)
    ...
    def reset(self) -> None:
        ...
        self._scroll_t = self._scroll_fine_x = 0
        self._vblank = self._sprite0_hit = False
        self._nmi_sent = False
        self._snap_v = self._snap_fine_x = 0
        self._snap_ctrl = self._snap_mask = 0
        self._snap_nt[:] = b"\x00" * 2048
        self._snap_pal[:] = b"\x00" * 32
        self._snap_oam[:] = b"\x00" * 256
        self._snap_valid = False
    ...
    def _nt_phys_buf(self, nt: bytearray, addr: int) -> int:
        addr = (addr - 0x2000) & 0x0FFF
        tb = addr // 0x400
        off = addr & 0x3FF
        m = self.bus.ppu_mirror if self.bus else (self.rom.mirror if self.rom else "H")
    ...
    def reg_read(self, addr: int) -> int:
        ...
        if r == 7:
            ...
            rendering = (self.regs[1] & 0x18) == 0x18
            if rendering and tmp < 0x3F00:
                ret = self.read_buffer & 0xFF
            elif tmp < 0x3F00:
    ...
    def reg_write(self, addr: int, val: int) -> None:
        ...
        if r == 0:
            ...
            if self.w == 0:
                ...
                self.w = 0
                self._scroll_t = self.t
                self._scroll_fine_x = self._fine_x
        elif r == 6:
            if self.w == 0:
    ...
    def snapshot_visible(self) -> None:
        self._snap_nt[:] = self.nametable
        self._snap_pal[:] = self.palette
        self._snap_oam[:] = self.oam
        self._snap_v = self._scroll_t
        self._snap_fine_x = self._scroll_fine_x
    ...
    def _get_bg_tile(self, table: int, tile_id: int, subpal: int, pal: bytearray | None = None):
        base = (table & 1) * 0x1000 + (tile_id & 0xFF) * 16
        pal = pal if pal is not None else self.palette
        bd = pal[0] & 0x3F
        pal_base = (subpal & 3) << 2
        col = bytearray(64)
        raw = bytearray(64)
        for row in range(8):
            ...
            for c in range(8):
                ...
                raw[ro + c] = bit
                pa = self._pal_addr(pal_base + bit)
                col[ro + c] = bd if bit == 0 else (pal[pa] & 0x3F)
        return bytes(col), bytes(raw)

    def render_rgb(self) -> bytes | None:
        if not self.rom or not self._snap_valid: return None
        ...
        nt = self._snap_nt
        pal_ram = self._snap_pal
        oam = self._snap_oam
        nt_phys = self._nt_phys_buf
        bd_idx = pal_ram[0] & 0x3F
        ...
        if bg_on:
            for py in range(H):
                ...
                for tcv in range(33):
                    ...
                    nt_i = nt_y * 2 + nt_x
                    nt_base = nt_i * 0x400
                    tile_id = nt[nt_phys(nt, 0x2000 + nt_base + tr * 32 + tc)]
                    attr = nt[nt_phys(nt, 0x2000 + nt_base + 0x3C0 + (tr >> 2) * 8 + (tc >> 2))]
                    shift = ((tr & 2) << 1) | (tc & 2)
                    subpal = (attr >> shift) & 3
                    col, raw = self._get_bg_tile(bg_tbl, tile_id, subpal, pal_ram)
                    src = fy * 8
                    dp = tcv * 8
            ...
            for py in range(H):
                ...
                for px in range(W):
                    o = ro + px * 3
                    buf[o] = bd_r; buf[o + 1] = bd_g; buf[o + 2] = bd_b

        if sp_on:
            chr_byte = self._chr_byte
            for si in range(63, -1, -1):
                base = si * 4
                if oam[base] >= 0xEF: continue
                oy = oam[base] + 1
                tile = oam[base + 1]
                attr = oam[base + 2]
                ox = oam[base + 3]
                ...
                for ry in range(sp_h):
                    ...
                    if tall:
                        if row < 8: tb = bank + tile_base * 16 + row
                        else:       tb = bank + (tile_base + 1) * 16 + (row - 8)
                    else:           tb = bank + tile_base * 16 + row
                    lo = chr_byte(tb)
                    hi = chr_byte(tb + 8)
                    py_curr = oy + ry
                    if py_curr < 0 or py_curr >= H: continue
                    row_off = py_curr * W
                    for fx in range(8):
                        ...
                        if pv == 0: continue
                        px_curr = ox + fx
                        if px_curr < 0 or px_curr >= W: continue
                        if behind and bg_pix[row_off + px_curr] != 0: continue
                        pa = self._pal_addr(0x10 + (spal << 2) + pv)
                        ci = (pal_ram[pa] & 0x3F) * 3
                        q = (row_off + px_curr) * 3
                        buf[q] = PAL_RGB[ci]; buf[q + 1] = PAL_RGB[ci + 1]; buf[q + 2] = PAL_RGB[ci + 2]
        return bytes(buf)
...
class NES:
    ...
    def load(self, path: str):
        if self.rom and self.rom.battery and self.bus.mapper:
            self.bus.mapper.save_battery(_battery_sav_path(self.rom.path))
        self.rom = INES(path)
    ...
    def power_on(self):
        ...
        if self.bus.mapper:
            ...
            if self.rom and self.rom.battery:
                self.bus.mapper.load_battery(_battery_sav_path(self.rom.path))
        self.cpu.reset()
...
class App:
    ...
    def close_rom(self):
        self.pause()
        if self.nes.rom and self.nes.rom.battery and self.nes.bus.mapper:
            self.nes.bus.mapper.save_battery(_battery_sav_path(self.nes.rom.path))
        self.nes = NES()
